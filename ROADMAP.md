...
- [ ] Build a shared sign up / log in flow (Supabase Auth) for both volunteers and org admins, differing only by role selected at signup
- [ ] Add an `is_verified` boolean to the `organizations` table (default false)
- [ ] Update RLS so only verified org owners (`is_verified = true`) can insert/edit opportunities
